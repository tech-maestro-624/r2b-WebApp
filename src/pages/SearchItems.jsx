...
const SearchItems = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.colors.background, color: theme.colors.text, fontFamily: 'Poppins, Arial, sans-serif' }}>
                ...
                >
                  {((rest.restaurant && rest.restaurant.image) || rest.image) && (
                    <Box sx={{ width: '100%', height: 160, overflow: 'hidden', borderTopLeftRadius: 12, borderTopRightRadius: 12 }}>
                      <img src={rest.restaurant?.image || rest.image} alt={(rest.name || rest.branchName) + ' cover'} style={{ width: '100%', height: '100%', objectFit: 'cover', borderTopLeftRadius: 12, borderTopRightRadius: 12 }} />
                    </Box>
                  )}
                  <Box sx={{ p: 2, pt: 1.5, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', gap: 0.5, alignItems: 'center', textAlign: 'center' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1, justifyContent: 'center', width: '100%' }}>
                      <Typography sx={{ fontSize: 20, fontWeight: 700, flex: 1, color: theme.colors.text, fontFamily: 'Poppins, Arial, sans-serif', lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textAlign: 'center' }}>{rest.branchName || rest.name}</Typography>
